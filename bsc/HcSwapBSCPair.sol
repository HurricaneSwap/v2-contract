...
contract HcSwapBSCPair is IUniswapV2Pair, HcSwapERC20 {
    ...
    function owner() public view returns (address){
        return IHcSwapBSCFactory(factory).owner();
    }

    modifier onlyOwner() {
        require(owner() == msg.sender,"HcSwap: NOT OWNER");
        _;
    }
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IHcSwapBSCFactory(factory).feeTo();
    ...
    // only for cross bridge 
    function directlyMint(uint liquidity, address to) external onlyOwner lock {
        _mint(to,liquidity);
        _sync();
    }

    // only for cross bridge 
    function directlyBurn(uint liquidity, address from, address to, uint amount0, uint amount1) external onlyOwner lock {
    ...
    // only for cross bridge
    function directlySync(uint256 amount0,uint256 amount1) external onlyOwner lock{
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savings
        _safeTransfer(_token0, msg.sender, IERC20(_token0).balanceOf(address(this)).sub(amount0));
        _safeTransfer(_token1, msg.sender, IERC20(_token1).balanceOf(address(this)).sub(amount1));
        require(amount0.mul(amount1) >= uint(reserve0).mul(reserve1),"HcSwap::directlySync K");
        _sync();
    }
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external onlyOwner lock {
        ...
        uint balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(5)); // 0.5% Fee
        uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(5)); // 0.5% Fee
